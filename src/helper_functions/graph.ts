...
export async function selectNode(id: string): Promise<void> {
  ...
  if(selectedNode && graphState.selected) {
    ...
    graphState.selected.outgoing = graphState.graph.edges.filter(edge => edge.source === id);
    graphState.selected.incoming = graphState.graph.edges.filter(edge => edge.target === id);
    setGraphState(graphState);
  }
}

export async function selectEdge(sourceId: string, targetId: string): Promise<void> {
  const graphState = await getGraphState();
  const selectedEdge = graphState.graph.edges.find(edge => edge.source === sourceId && edge.target === targetId);
  if(selectedEdge && graphState.selected) {
    ...
    graphState.selected.neighbors = graphState.graph.nodes.filter(node => node.id !== sourceId && node.id !== targetId);
    graphState.selected.outgoing = graphState.graph.edges.filter(edge => edge.source === sourceId);
    graphState.selected.incoming = graphState.graph.edges.filter(edge => edge.target === targetId);
    setGraphState(graphState);
  }
}
